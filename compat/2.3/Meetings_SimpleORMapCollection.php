 ...
 * @category    Stud.IP
*/
class Meetings_SimpleORMapCollection extends Meetings_SimpleCollection
{
    ...
    {
        $ret = new Meetings_SimpleORMapCollection();
        if (count($data)) {
            $first = current($data);
            if ($first instanceof Meetings_SimpleORMap) {
     ...
     */
    function __construct(Closure $finder = null, Array $options = null, Meetings_SimpleORMap $record = null)
    {
     ...
     */
    function setRelatedRecord(Meetings_SimpleORMap $record)
    {
        $this->related_record = $record;
    }
